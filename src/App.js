...
class App extends React.Component {
  ...
  loadUser = (data) => {
    console.log(data, data.diaryname);
    this.setState({
      user: {
        id: data.id,
        name: data.name,
        ...
        {this.state.route === "signIn" ? (
          ...
          <div className="full ">
            <div className="center Diaryname">
              ...
              <h1 id="email" style={{ display: "none" }}>
                {this.state.user.email}
              </h1>
              <IconContext.Provider
                value={{ className: " pointer sign-out-icon" }}
              >
                <div
                  className="sign-out-icon-container"
            ...
            <div className="App  w-100 ">
              <Diary
                ...
                textColor={this.state.user.textColor}
                diaryName={this.state.user.diaryName}
                changeTheme={this.changeTheme}
                changeDiaryName={this.changeDiaryName}
              />
            </div>
          </div>
        )}
